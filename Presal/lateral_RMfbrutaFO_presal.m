...
disp('Teste de remoção de variação lateral de velocidades');
disp('ESTE PROGRAMA AINDA NÃO SE ENCONTRA EFETIVO! a INVESTIGAR...');
...
linf=ixtrace_co-round(nl/100); % Melhor resultado: nl/2, nl/10
lup=ixtrace_co+round(nl/100);  % Melhor resultado: nl/2, nl/10
...
if(iteta2<=nt);
migi5(it,ix)=migi5(it,ix)+migi_aux(iteta2,ix2)*dx;
else;end;

end; % FIM DO LAÇO MAIS INTERNO

else;migi5(it,ix)=0.0;end; % FIM DO LAÇO DO TEMPO

end;

else;migi5(it,ix)=0.0;end; % FIM DO LAÇO DA VELOCIDADE NULA

end;
end;

outi=real(migi5);
save presal_lateral_out.dat outi -ascii;
clear outi;

figure;
imagesc([1:nx]*dx,[1:nt]*dt,real(migi5(1:nt,:)));colormap gray;grid;
xlabel('Distância (m)');ylabel('Tempo (s)');
title('Pré-sal remigrado - força bruta');