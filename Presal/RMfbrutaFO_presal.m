disp('Este prorama deve ser rodado após terceiro_presal');
...
teta2=real(sqrt(tempo^2-(4*h^2/vm^2))*sqrt(1-((x2-xtrace)^2/limite^2)));
end;
...
% == Equalização de amplitudes até nt=500 (2s) == %
%for ix=1:nx;
%    for it=500:1250; t=dt+(it-1)*dt;
%        migi4(it,ix)=migi4(it,ix)*t^(2.21);
%    end;
%end;
% =============================================== %

outi=real(migi4);
save presal_remigfb.dat outi -ascii;
clear outi;
