...
%               work here such as 'g' or 'green' or [0 1 0]. See Matlab
%reference manual
...
% ************** default = 1 *********************
%
% G.F. Margrave, PhD, P. Geoph., April 1994
...
%
        ...
        %               2) Find the boundary points of the groups of 1). 
        %                  That is determine how the points fall into
        %                  contiguous groups and where these groups start 
        %                  and end.
        %               3) Find any groups of 1) which are single isolated
        %                       points and add an extra point so that the 
        %                       minimum group size is 2
        %               4) For all groups, interpolate in the zero
%crossings at the
        %                       beginning and end of the group and assign
%the first and
        %                       last points in the group to these values
        %               5) assign all other points in the group to the
%value 0 (or snot)
        ...
        if(test>1000*eps)
                ...
                % put snot on both ends. This forces the detection of
%isolated singular
                % points at the ends
                ...
                if ~isempty(ising)
                        ...
                        if  ~isempty(ising)
                                for k=1:length(ising)
                                        ...
                                        ss=[ss(1:ising(k)); ss(ising(k));
ss(ising(k)+1:length(ss))];
                                        tt=[tt(1:ising(k)); tt(ising(k));
tt(ising(k)+1:length(tt))];
                                        ising=ising+1;
                                end
                        end
                        % refind the points because the point count may
%have changed
                        %find all points which won't be shaded
                        il=find(ss<=snot);
                        ilow=[0;il;n+1]; %end conditions
