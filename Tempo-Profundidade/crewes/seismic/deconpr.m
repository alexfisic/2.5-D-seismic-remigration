...
if((m-1)*(n-1)==0)
    ...
    % design prediction operator
     x= predict(trdsign,nop,nlag,stab);%solves with levrec
     %x= predict_alt(trdsign,nop,nlag,stab);%Forms normal equations and uses pinv
    % form the predicted part of trin
     trinpr= conv(trin,x);
    % delay and subtract
     trout= trin(nlag+1:length(trin))-trinpr(1:length(trin)-nlag);
     if(n==1)
    ...
    for k=1:n
        x= predict(trdsign(:,k),nop(k),nlag(k),stab(k));%solves with levrec
        %x= predict_alt(trdsign(:,k),nop(k),nlag(k),stab(k));%Forms normal equations and uses pinv
        % form the predicted part of trin
         trinpr= conv(trin(:,k),x);
        % delay and subtract
         tmp= trin(nlag(k)+1:length(trin),k)-trinpr(1:length(trin)-nlag(k));
