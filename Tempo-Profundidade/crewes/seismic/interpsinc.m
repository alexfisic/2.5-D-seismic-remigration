function trout=interpsinc(t,trin,tout,sizetable)
% INTERPSINC: sinc function interpolation for time series without nan's
%
%  trout=interpsinc(trin,t,tout,sizetable)
%
% INTERPSINC performs 8 point sinc function interpolation using a 
% design for the approximate sinc function due to Dave Hale. This is
% identical to sinci except for the order of the input arguments. The order
% here is consistent with Matlab interpolators such as interp1.
%
% t= time coordinate vector for input trace. Trace must be regularly
%    sampled. INTERPSINC uses only the first two points in t.
% trin= input trace
