function phi=constphase(s1,s2,flag)
% CONSTPHASE: find the best constant phase rotation to match one signal to another
%
% phi=constphase(s1,s2,flag)
%
% Given two signals s1 and s2, this function finds the constant phase rotation which, when applied
% to s1, minimizes the L2 norm of the difference: s2-phase_rotated(s1) . ("Constant phase" means the
% phase is the same for all frequencies.) This method uses the analytic solution.
%
% s1 ... input time series to be rotated
% s2 ... input time series to match to rotated s1
% ************* default is a unit impulse at the center of s1 **************
