...
%
% Description 
%   AEC performs an automatic amplitude adjustment. An all zeros trace will be all zeros on output.
%
% Method
...
%               trace mode. If profileflag is 1, then the hilbert
...
%   ma        = maximum absolute value of the corrected trace before
%               normalization to 1.
% NOTES:
...
if numel(trin) ~= length(trin)
    ...
    if(profileflag)
        ...
        ma=zeros(1,ntr);
        envsm=average_envelope(trin,sampint,op_length,trip);
        maa=0;%max absolute amplitude on whole gather
        for k=1:ntr
            tmp=trin(:,k)./envsm;
            ma(k)=max(abs(tmp));
            if(ma(k)>maa);maa=ma(k);end
            trout(:,k)=tmp;
            %ma(k)=max(abs(tmp));
            %trout(:,k)=tmp/ma(k);
        end
    ...
    %these next lines are a bad idea if looping over a gather. So they are commented out for now
%     if(ma)
%         trout=trout/ma;
%     end
end
...
envsm=conv(env,triang(nop))/sum(triang(nop));
