function [trcout,multiplier]=tvbalans(trcin,t,trcref,tref,twin,tinc)
% TVBALANS balances the traces to match the amplitude of the well using
% gaussian time windows
%
% trcout=tvbalans(trcin,t,trcref,tref,twin,tinc)
%
...
%
% trcout= time-variant balanced trace
% multiplier= same size matrix as trcin containing the trace multipliers.
%       That is, trcout(:,k)=trcin(:,k).*multiplier(:,k); where k is trace
%       number.
%
% by G.F. Margrave and H.J.E. Lloyd 2012, 2013
%
...
for kk=1:size(trcin,2)%loop over traces
    for k=1:nwin%loop over windows
        ...
        %amp_model(k)=sum(gwin.*trcin(iuse,kk).*trcref)/sum((gwin.*trcin(iuse,kk)).^2);
        %waitbar(k/nwin,wbar);
    end
    %expand the amplitude model
    multiplier=interpextrap(t_model,amp_model,t,0);
    %apply the amplitude model to balance the trace
