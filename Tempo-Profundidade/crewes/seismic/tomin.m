function [wavemin,wtomin]=tomin(wavelet,stab)
...
wavemin=balans(wavemin,wavelet);
%   %make another all-pass wavelet whose phase is the difference between
%   %wavemin and wavelet
  WM=fft(wavemin);
  W=fft(wavelet);
  wtomin=real(ifft(conj(W./WM)));
%make a match filter that converts wavelet to wavemin
%     tw=.001*(0:length(wavelet)-1);
%     wtomin=match(wavelet,wavemin,tw,max(tw)/2,1);
