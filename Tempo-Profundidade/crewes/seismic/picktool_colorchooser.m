function selectedcolor=picktool_colorchooser(names,oldcolor,colors_inuse,transfer,posxy)
%COLORCHOOSER ... interactive GUI tool for color choice
% 
% selectedcolor=colorchooser(hfig,oldcolor,colors_inuse,transfer)
% 
% names = list of horizon names
% oldcolor ... rgb triplet specifying the current color that is to be changed.
% colors_inuse ... cell array of colors that are currently in use. Each entry must be an rgb triple.
...
if(strcmp(action,'init'))
   ...
   ht=wid;
   ynow=1-ht-.1;
   coloraxes(hdial,[xnow,ynow,wid,ht],10,'colorchooser(''clickcolor'');',oldcolor);
...
for k=1:n2
    for j=1:n2
        ...
        if(ipatch==iselected)
            ...
            x2=xx+[-1;1;1;-1]/dx/4;
            y2=yy+[-1;-1;1;1;]*dy/4;
            set(hp(ipatch),'linewidth',2,'xdata',x2,'ydata',y2,'zdata',ones(4,1),'userdata',[xx yy]);
        end
        if(ipatch==npatches)
            break;
        end
