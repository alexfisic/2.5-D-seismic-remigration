...
%   It only makes sense to program a variable design gate if you set itr=0 (trace-by-trace
%   operation).
...
if(length(xdesign)~=length(tstart))
    error('xd and tstart must have the same length');
end
if(length(xdesign)~=length(tend))
    error('xd and tend must have the same length');
end
if(length(xdesign)==1)
    xdesign=[1 size(stack,2)];
