...
elseif(strcmp(action,'changecolor')||strcmp(action,'changecolor2'))
    if(strcmp(action,'changecolor'))
        hfig=gcf;
    else
        hdial=gcf;
        hfig=hdial.UserData;
    end
    %test for cancel with 'changecolor2'
    if(strcmp(action,'changecolor2'))
        results=picktool_getcolor('getresult');
        delete(hdial);
        if(isempty(results))
            return;
        end
    ...
    hstopsave=findobj(hfig,'tag','stopsave');
    hstart=findobj(hfig,'tag','start');
    udat=get(hstopsave,'userdata');
    pickstruc=udat{1};
    hornames=pickstruc.hornames;
    horcolors=pickstruc.horcolors;
    ...
    if(strcmp(action,'changecolor'))
        ...
        picktool_getcolor(hornames,horcolors{1},horcolors,posxy,'picktool(''changecolor2'');');
        set(gcf,'userdata',hfig);
        return;
    end
    ...
    udat{1}=pickstruc;
    set(hstopsave,'userdata',udat);
elseif(strcmp(action,'newhor')||strcmp(action,'repick')||strcmp(action,'newhorB'))
    if(~strcmp(action,'newhorB'))
        hfig=gcf;
    else
        hdial=gcf;
        hfig=hdial.UserData;
    ...
    if(strcmp(action,'newhor')||strcmp(action,'newhorB'))
        ...
        nhors=length(hornames);
        hornames{nhors}='';
        horcolors{nhors}=ones(1,3);
        udat2=get(hstart,'userdata');
        anchorarray=udat2{1};
        if(strcmp(action,'newhor'))
            pos=get(hfig,'position');
            posdialog=[pos(1)+.5*pos(3),pos(2)+.5*pos(4)];
            picktool_getcolor(hornames,nan,horcolors,posdialog,'picktool(''newhorB'');');
            set(gcf,'userdata',hfig);
            return;
        else
            results=picktool_getcolor('getresult');
        ...
        picktool('stop');%turn off editing, this will also save the window button functions
        nhors=length(newcolors);
...
elseif(strcmp(action,'newhor2')||strcmp(action,'repick2'))
    %ask if the autopick was worth keeping
        ...
        case 'Yes'
            ...
            if(sum(ians)>0)
                ...
                if(~strcmp(choice,'do nothing'))
                    [~,tpe]=fillextend(xp,tp,xseis,choice);
                    [~,ape]=fillextend(xp,ap,xseis,choice);
...
elseif(strcmp(action,'delete'))
    ...
    if(isempty(pickstruc.hornames'))
        msgbox({'You have no horizons!!','You need at least one horizon in order to delete something...'})
        return;
    end
...
elseif(strcmp(action,'showtype'))
    ...
    d=figure('position',[pos(1)+.5*pos(3),pos(2)+.5*pos(4), 400 300],'numbertitle','off','menubar','none');
...
elseif(strcmp(action,'legend'))
    ...
    if(state==1)
        ...
        legend(anch,pickstruc.hornames,'color',.9*ones(1,3));
...
%sort xseis into ascending order
xseis=sort(xseis);
[xp,isort]=sort(xp);
tp=tp(isort);
...
%     thisfield=getfield(pickstruc,fields{k});
    thisfield=pickstruc.(fields{k});
    thisfield(ihors)=[];
%     pickstruc=setfield(pickstruc,fields{k},thisfield);
    pickstruc.(fields{k})=thisfield;
end
end
