...
% f1 ... lowest trustworthy signal frequency. May be either a scalar is a
%       constant value sufficies or a vector the same length as zq giving
%       depth-dependent values.
% f2 ... highest trustworthy signal frequency. May be either a scalar is a
%       constant value sufficies or a vector the same length as zq giving
%       depth-dependent values.
...
ind=find(test<=0);
if(~isempty(ind))
    error(['f1 exceeds f2 at depth ' num2str(zq(ind(1))) ' and possibly elsewhere'])
end
...
%     Qint(k)=Qave(k);
    ...
    if(t2<=t1)
        ...
        [Qint(k),CA(k)]=qestimator(w0,wk,t,t1,t2,twin,f1(k),f2(k),wintype,spectype,pflags(k),method,nan,p);
    end
end
% zqave=(z0+zq)/2;
