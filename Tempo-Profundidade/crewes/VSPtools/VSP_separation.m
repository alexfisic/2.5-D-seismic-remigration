...
vspflat=VSP_flatten(vsp,t,tp,1,mean(tp));
...
for k=1:npasses
    ...
    %fk filter to reject anything left that is downgoing but slower than
    %the first break
...
vspdown=VSP_flatten(vspdownflat,t,tp,-1,mean(tp));
rdown=VSP_flatten(vspflat,t,tp,-1,mean(tp));%residual downgoing wave
...
% 
% dtp=diff(tp);
% tpup=flipud([tp(end); tp(end)+cumsum(flipud(dtp))]);
tpup=tp;
vspupflat=VSP_flatten(vspup,t,tpup,-1);
