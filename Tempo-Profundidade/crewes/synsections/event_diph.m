...
for k=1:nc %loop over traces
    ...
    tk=sqrt(tnot.^2+((x(k)-xnot)/v).^2);%diffraction arrival time
    ak=anot.*tnot.*(tk.^(-1.5));%empirical amplitude
    ...
    if(ind2~=0)
        ...
        trctmp2=resamp(trctmp,ttmp,dt,[min(t) max(t)],0);
%         trctmp3=(trctmp2-min(trctmp2)+min(trctmp))*(max(trctmp)-min(trctmp));
        trctmp3=trctmp2;
        amat(:,k)=amat(:,k) + trctmp3/costheta;
