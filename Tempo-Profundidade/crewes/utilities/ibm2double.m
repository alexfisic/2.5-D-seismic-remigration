function d = ibm2double(u)
%
% function dibm = ibm2double(u)
% Where:
...
% fclose(fid)
%
% See also ibm2single, single2ibm, double2ibm, log16
%
...
if ~isa(u,'uint32')
    error('ibm2double: Input must be a 4-byte unsigned integer');
end

%get sign bit 0=(+), 1=(-), stored in bit 32
s = double(bitget(u,32));
...
e = double(bitshift(bitand(u,uint32(4278190080)),-24))-70.0; 

%get fraction
%hex2dec('00ffffff') = 16777215; hex2dec is slow!
d = double(bitand(u,uint32(16777215))); %mask exponent
...
end %end function d = ibm2double(u)