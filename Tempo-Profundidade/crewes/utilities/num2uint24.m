function d = num2uint24(d,fe)
% function u = num2uint24(d,fe)
% Where:
%     u = 3-byte unsigned integers stored as 8-bit unsigned integers
%     d = Any number(s) stored in any Matlab datatype. vectors and arrays
%         are OK
%    fe = file endian 'l' (little-endian) or 'b' (big-endian). Default is
%         computer endian (usually 'l')
%
% Example:
%
% fid = fopen('test.uint24,'w')
% u = num2uint24([7,8,9],'b');
% fwrite(fid,u,'uint8')
% fclose(fid)
%
% Authors: Kevin Hall, 2017, 2018
%
% See also num2int24, int24_2int32, uint24_2uint32
%
