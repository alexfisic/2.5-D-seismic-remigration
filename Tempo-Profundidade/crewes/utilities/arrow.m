function h=arrow(x,y,txt,kol,lw,ls,ahead,pflag)
% ARROW draws an arrow with a text label in the current axes
%
% h=arrow(x,y,txt,kol,lw,ls,ahead,pflag)
%
...
if(~isempty(txt))
    h4=text(xy(1,1),xy(2,1),txt,'color',kol);
else
    h4=[];
end
