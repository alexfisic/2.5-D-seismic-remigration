function [seisw,t]=afd_shotrec_alt(dx,dtstep,dt,tmax, ...
         model,xshot,zshot,xrec,zrec,wlet,tw,laplacian,boundary)
% AFD_SHOTREC_ALT ... makes finite difference shot records
%
% [seisw,t]=afd_shotrec_alt(dx,dtstep,dt,tmax,model,xshot,zshot,xrec,zrec,wlet,tw,laplacian,boundary)
%
%
...
% NOTE: dx and dtstep must satisfy the stability condition for the chosen Laplacian. See the
% discussion below.
...
% xshot = array of x coordinates of the desired shot points (in consistent units)
% zshot = array of z coordinates of the desired shot points (in consistent units)
...
if(denflag)
    disp('variable density modelling');
else
    disp('constant density modelling');
end
