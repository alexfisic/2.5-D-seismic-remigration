...
%       then this is just the same as snap2. If restarting a simulation, then this
...
if laplacian ==1
    if max(max(velocity))*delt/delx > 1/sqrt(2)
        error('Model is unstable:  max(velocity)*delt/delx MUST BE < 1/sqrt(2)');
    end
else
    if max(max(velocity))*delt/delx > sqrt(3/8)
        error('Model is unstable:  max(velocity)*delt/delx MUST BE < sqrt(3/8)');
    end
end
