...
%
% fignoz ... figure number or handle to get the depth (dip) picks from
% fignot ... figure number or handle to plot the times in
...
if(isnumeric(fignot))
    fignot=fignum2handle(fignot);
end
if(isnumeric(fignoz))
    fignoz=fignum2handle(fignoz);
end
...
%estimate maximum time for ray
tmax=3*max(RTT);

[npts,nc]=size(pts);
params={fignot, fignoz, .004, 2, tmax};
for k=1:npts
