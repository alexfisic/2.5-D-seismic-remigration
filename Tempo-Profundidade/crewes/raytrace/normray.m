...
% params ... cell array of length 5
% params{1} ... figure number or handle to plot in (time)
%       *********** default = don't plot ********
% params{2} ... figure number or handle to draw ray in depth
%       *********** default = don't plot ********
% params{3} ... time step
%       *********** default =.004 **********
...
if(nargin<4); params=cell(1,5); 
elseif (length(params)~=5)
	error('params must be specified with 5 entries')
end

if(isempty(params{3})); params{3} = .004; end
if(isempty(params{4})); params{4} = 2; end
if(isempty(params{5})) 
...
%plot if requested
if(~isempty(zfigno))
    if(isnumeric(zfigno)); zfigno=fignum2handle(zfigno); end
	figure(zfigno);
...
%plot if requested
if(~isempty(tfigno))
    if(isnumeric(tfigno)); tfigno=fignum2handle(tfigno); end
	figure(tfigno);
	delx=6*RTDG;
