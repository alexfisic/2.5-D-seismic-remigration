function [t,p,L,raycoord]=traceray_ps(vp,zp,vs,zs,zsrc,zr,zd,x,xcap,pfan,itermax,optflag,pflag,dflag)
% TRACERAY_PS: traces a P-S (or S-P) reflection for v(z)
%
% [t,p,L,raycoord]=traceray_ps(vp,zp,vs,zs,zsrc,zr,zd,x,xcap,pfan,itermax,optflag,pflag,dflag)
%
...
if(nargout>3)
    ...
    for k=1:length(p)
        ...
        sn=flipud(vsu(1:end-1))*p(k);
        cs=sqrt(1-sn.^2);%cos(theta)
        xrayu=xrayd(end)+[0;cumsum(abs(diff(flipud(zsu))).*sn./cs)];
        xray=[xrayd;xrayu];
        zray=[zpd;flipud(zsu)];
