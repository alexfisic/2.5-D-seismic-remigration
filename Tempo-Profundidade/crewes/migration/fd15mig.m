...
%
% FD15MIG is a 15 degree finite difference time migration routine.
%
...
% dtau ... a scalar indicate the step length in time, unit is millisecond.
%
%
% OUTPUT arguments
%
...
% Modified G.F. Margrave, CREWES, June 2000
%
...
%  ---- check dtau ----
vmax = max(max(aryvel));
%if dtau >= 8.*dx^2/vmax^2/dt
%	error( ' the step dtau should be less ');
%end
...
disp([int2str(ndown) ' steps required'])
...
%disp (['The step # ' int2str(idown) ' in ' int2str(ndown)]);
	...
	for tlev = nsamp-1:-1:top1
		taueff = dtau;
		if (tlev < top2 )
			taueff = dtau*(tlev - top1)/ktausm;
		end
		aryo(1,:) = arymig(tlev+1,:);
		vel = 0.5*aryvel(round((top1+top2)/2)+1, :);
	...
	if(rem(idown,10)==0)
		disp(['completed step ' int2str(idown) ' of ' int2str(ndown)])
	end
end
		
%disp(['Total floating point operations --' int2str(flops)]);
toc