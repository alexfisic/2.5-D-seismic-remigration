function mdata=gaz_mig(fdata,f,parms,dx,dz)
%mdata=gaz_mig(fdata,f,parms,dx,dz);
%
%***prestack gazdag depth migration***
%
%To see an example, type 'gaz_salt_psdm_script' in matlab.
%
...
%parms...velocity model (m/s). A vector.
%dx...trace spacing (m).
%dz...depth interval (m).
%
%R. J. Ferguson, 2009
...
if Nx~=1; error('x axis wrong');end
%***************
...
rtemp=zeros(rd,cd);
for j=1:Nz-1
	disp([' gazdag prestack mig working on depth ',num2str(j),' of ',num2str(Nz)])
	ftemp=ips(fdata,f,dx,parms(j),dz);
	fdata=fft(ftemp,[],2);
