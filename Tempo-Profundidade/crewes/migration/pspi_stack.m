function [zosmig,checkinfo]=pspi_stack(zos,t,x,vel,xv,zv,frange,zcheck,irezero)
% PSPI_STACK: zos (zero offset section) migration by the PSPI algorithm
%
% [zosmig,checkinfo]=pspi_stack(zos,t,x,vel,xv,zv,frange,zcheck,irezero)
%
...
% NOTE: size(vel) must equal [length(zv), length(xv)]. Also xv(2)-xv(1) must equal x(2)-x(1).
...
%       plotted using xv and zv: seisplot(zosmig,zv,xv,'PSPI migration')
% checkinfo...cell array of checkpoint info. Currently of length 3
% checkinfo{1}=exzos ...cell array of extrapolated sections, one for each zcheck.  These
%       are downward continued zos's that simulate what would have been
%       recorded at each depth in zcheck.
% checkinfo{2}=pmigs ... cell array of partial migrations, same length as exzos
% checkinfo{3}=zcheck ... actual depths for output exzos. May be different from input zcheck
...
%   seisplot(exzos{1},tzos,xzos,['Extrapolated section datum=' int2str(zcheck(1)) ' m'])
...
for j=1:Nz-1
    ...
    if(j==jcheck)
        ncheck=ncheck+1;
        tmp=zos2t(zosfk,f,dx);
        exzos{ncheck}=tmp(1:nt,1:nx);
        pmigs{ncheck}=zosmig(:,1:nx);
