...
usesmooth=1;%0 to migrate with the exact model, 1 to use the smoothed model

if(usesmooth==1)
    zcheck=0:100:2000;
    % [zosmig,exzos]=pspi_stack(seisfiltt,t,x,velt,x,z,[5 50],zcheck);
    [zosmigsmo,exzossmo]=pspi_stack(seisfiltt,tt,xt,veltsmo,xt,zt,[5 50],zcheck);
    ...
    zcheck=0:100:2000;
    [zosmig,exzos]=pspi_stack(seisfiltt,tt,xt,velt,xt,zt,[5 50],zcheck);
    
    plotimage(zosmig,xt,zt);
    ...
    %load the extrapolations into plotgathers
    plotgathers(exzos{1},xs,ts,'distance (m)','time (s)',titles);
end
