function mdata=gaz_zero_mig(fdata,f,parms,dx,dz)
%mdata=gaz_zero_mig(fdata,f,parms,dx,dz);
%
%***generalized screen zero-offset migration***
%
%To see an example, type 'gaz_salt_zero_script' in matlab.
%
...
temp=zeros(rd,cd);
for j=1:Nz-1
	disp([' gaz zero mig working on depth ',num2str(j),' of ',num2str(Nz)])
	temp=ips(fdata,f,dx,parms(j),dz);
	fdata=fft(temp,[],2);
