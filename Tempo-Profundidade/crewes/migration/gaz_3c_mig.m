function mdata=gaz_3c_mig(fdata,f,parms,s_parms,dx,dz)
%mdata=gaz_3c_mig(fdata,f,parms,s_parms,dx,dz)
%
%***prestack gazdag depth 3C migration***
%
%To see an example, type 'gaz_salt_3c_psdm_script' in matlab.
%
...
%parms...p-velocity model (m/s). A vector.
%s_parms...s-velocity model (m/s). A vector.
%dx...trace spacing (m).
%dz...depth interval (m).
%
%R. J. Ferguson, 2009
...
if or(Nxs~=1,Nx~=1); error('x axis wrong');end
if and(Nxs~=Nx,Nzs~=Nz); error('p and s model dimensions must match');end
%***************
...
rtemp=zeros(rd,cd);
for j=1:Nz-1
	disp([' gazdag prestack 3c mig working on depth ',num2str(j),' of ',num2str(Nz)])
	ftemp=ips(fdata,f,dx,s_parms(j),dz);
	fdata=fft(ftemp,[],2);
	stemp=ips(fsou,f,dx,parms(j),-dz);
	fsou=fft(stemp,[],2);
