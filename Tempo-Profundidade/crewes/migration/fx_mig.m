function mdata=fx_mig(fdata,f,parms,dx,dz,dip)
%mdata=fx_mig(fdata,f,parms,dx,dz,dip);
%
%***prestack 80 degree fx depth migration***
%
%To see an example, type 'fx_salt_psdm_script' in matlab.
%
...
rtemp=zeros(rd,cd);
for j=1:Nz-1
	disp([' fx prestack mig working on depth ',num2str(j),' of ',num2str(Nz)])
	ftemp=fx_ips(fdata,f,dx,parms(j,:),dz,dip);
	fdata=ips(ftemp,f,dx,max(parms(j,:))/sin(pi*dip/180),dz);%stabilize
	ftemp=ips(fdata,f,dx,max(parms(j,:))/sin(pi*dip/180),-dz);%stabilize
	fdata=fft(ftemp,[],2);
	stemp=fx_ips(fsou,f,dx,parms(j,:),-dz,dip);
	fsou=ips(stemp,f,dx,max(parms(j,:))/sin(pi*dip/180),dz);%stabilize
	stemp=ips(fsou,f,dx,max(parms(j,:))/sin(pi*dip/180),-dz);%stabilize
	fsou=fft(stemp,[],2);
