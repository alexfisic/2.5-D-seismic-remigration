...
[rm cm]=size(model);
model=3000+0*model(:,round(cm/2)+1);
s_model=model/2;
load shot_impulse
dt=t(2);
dx=xrec(2);
...
%***migrate***
disp('Your output file will be "gaz_salt_3c_mig"')
pause(3)
gaz_3c_psdm=gaz_3c_mig(fseis,f,model,s_model,dx,dz);
%*************

%***output***
save gaz_salt_3c_mig gaz_3c_psdm
%************