function [arymig,tmig,xmig,gath]=kirk_mig(aryin,aryvel,dt,dx,params,xgath)
...
%    params(4-6) : angle limit in degrees
%    params(4) ... maximum dip limit
%                 default = 60
...
if( npad > 0)
	...
	if ibcfilter
        %this looks crazy. It's probably wrong. Not sure what is the intent.
		for j=1:npad
...
for ktr=trtarget 			% ktr--the location of output trace
    ...
    if(rem(kmig,20)==0)
        ...
        timeremaining=(timeused/kmig)*(ntrtarget-kmig);
	    disp([' Migrated trace no.' ,int2str(kmig) ,' of ' int2str(ntrtarget) ', The input traces in aperture : ' ,int2str(length(truse))]);
...
%there is some kind og low-frequency horizontal artefact at the beginning and end of the
%section. Son't know why so just muting it with an mwindow.

mw=mwindow(length(t),10);
arymig=arymig.*mw(:,ones(size(x)));
