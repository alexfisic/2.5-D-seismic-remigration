function mdata=ss_zero_mig(fdata,f,parms,dx,dz)
%mdata=ss_zero_mig(fdata,f,parms,dx,dz);
%
%***generalized screen zero-offset migration***
%
%To see an example, type 'ss_salt_zero_script' in matlab.
%
...
temp=zeros(rd,cd);
for j=1:Nz-1
	disp([' ss zero mig working on depth ',num2str(j),' of ',num2str(Nz)])
	temp=ss_ips(fdata,f,dx,parms(j,:),dz);
	fdata=fft(temp,[],2);
