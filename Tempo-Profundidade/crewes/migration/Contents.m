...
%  DEMO_DEPTHMIG -  post stack depth migration with PSPI_STACK (similar to the next)
%  DEMO_PSPI_STACK -  script to demonstrate post-stack depth migration with PSPI_STACK
%  VZ_MOD_SIM -  script to demo vz_fkmod (v(z) modelling with f-k method)
%  VZ_FK_SIM -  script to demo vz_fkmig (f-k migration for v(z))
...
%  TEST_RTM_STACK -  script to demo rtm_stack.
%
% Extrapolation
%  PS_STEP - take a single depth step by phase shift
% 
...
%  RTM_STACK -  post-stack reverse-time migration (2D)
...
%  WAVEFRONTMIG -  migration by wavefront superposition (constant velocity only)
%
% Prestack migration
...
%  RTM_SHOT - prestack reverse-time migration of a shot record (2D)
%
