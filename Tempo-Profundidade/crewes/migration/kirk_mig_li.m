function [arymig,tmig,xmig]=kirk_mig(aryin,aryvel,dt,dx,params)
...
if ibcfilter 
	...
	arycum=aryin;
	for isamp = nsamp:-1:2
		arycum(isamp,:)=sum(aryin(1:isamp,:));
	end
end
...
for ktr=trtarget 			% ktr--the location of output trace
	...
	for kaper=1:length(truse)
		...
		if abs(truse(kaper)-ktr)*dx > 0.5*aper
			ccoef = coef1( round(abs(truse(kaper)-ktr)-traper0+1) );
		end
		if abs(1-ccoef) > 0.05
			tmp = tmp .* ccoef;
		end
...
tottime=etime(clock,tstart);
%totflops=flops;
disp(['Total run time --' int2str(tottime)]);
%disp(['Total floating operation --' int2str(totflops)]);