function mdata=gs_mig(fdata,f,parms,dx,dz,dip)
%mdata=gs_mig(fdata,f,parms,dx,dz,dip);
%
%***prestack generalized screen depth migration***
%
...
%dx...trace spacing (m).
%dz...depth interval (m).
%dip...maximum dip for stabilization (dip < 90 degrees)
%
%R. J. Ferguson, 2009
%
...
rtemp=zeros(rd,cd);
for j=1:Nz-1
	disp([' gs prestack mig working on depth ',num2str(j),' of ',num2str(Nz)])
	ftemp=gs_ips(fdata,f,dx,parms(j,:),dz);
	fdata=ips(ftemp,f,dx,max(parms(j,:))/sin(pi*dip/180),dz);%stabilize
	ftemp=ips(fdata,f,dx,max(parms(j,:))/sin(pi*dip/180),-dz);%stabilize
	fdata=fft(ftemp,[],2);
	stemp=gs_ips(fsou,f,dx,parms(j,:),-dz);
