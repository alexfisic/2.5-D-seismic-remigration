...
zcheck=0:100:2000;
[zosmig_depth,chkinfo]=pspi_stack(seisfiltt,t,x,velt,x,zv,[5 50],zcheck);
seisplot(zosmig_depth,zv,x,'Depth migration of Thrust');
titles1=cell(size(chkinfo));
titles2=titles1;
xs=(x(2)-x(1))*(0:size(chkinfo{1}{1},2)-1);
ts=(t(2)-t(1))*(0:size(chkinfo{1}{1},1)-1);
for k=1:length(chkinfo{1})
    titles1{k}=['Extrapolated to ' int2str(zcheck(k))];
    titles2{k}=['Migration to ' int2str(zcheck(k))];
end
save thrustdata

%load the extrapolations into plotgathers
plotgathers(chkinfo{1},xs,ts,'distance (m)','time (s)',titles1);
%load the migrations into plotgathers
plotgathers(chkinfo{2},xs,zv,'distance (m)','time (s)',titles2);
