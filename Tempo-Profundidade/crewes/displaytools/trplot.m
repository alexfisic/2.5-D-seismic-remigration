function [htraces,hnames,dcshifts]=trplot(times,traces,varargin)
% TRPLOT ... make a nice trace plot
%
% [htraces,hnames]=trplot(times,traces,varargin)
%
% Plots in the current axes an array of traces with the time axis horizontal (by default) or
% vertical. Options exist to shift the traces vertically, resample for smooth curves, normalize, and
% so on. This is meant as a time-domain partner for dbspec.
%
...
%              the normalization window. The normalization process will only look at amplitudes in
%              this window. The trace separation will also be determined by the maximum amplitude in
%              this window.
% ************ default [t(1) t(end)] ***********
% 'yaxis' ... 'n' ... means don't show numerical labels on y tick marks (default)
%             'y' ... means do show numerical labels
% ************ default is 'n' (don't show) ****************
...
%            supplied name is placed at the end, middle, or beginning of the trace.
% ************ default is 'end' ***************
% 'namesalign' ... Choose one of 'left','right','center'. Means that
%            the name is either left, center, or right justified with the end, beginning, or middle.
...
%               names in the direction orthogonal to the trace. Specified as a fraction of the trace
%               spacing. A value of 0 gets the default behaviour while 1 shifts the name up to the
%               next trace.
% ************ default is 0 *************
% 'resample' ... n ... the traces will be resampled (sinc interpolation) 
%             to have n times as many samples as input. So n=1 means no resampling, n=2 means twice
%             as many samples, and so on. May be a decimal number.
...
% ************* Default is 9 ******************
% 'linewidths' ... vector of values, one per trace giving the linewidths.
%               A value of 1 is the standard linewidth while a value of 2 is a double width. If a
%               single value is given it is assumed to apply to all traces. If more than 1 value,
%               then there must be one value per trace.
...
%               marker. Common choices are '*', '.', 'o', and 'none'. A value of '' (empty) is the
%               same as the default 'none'
% ************* Default is 'none' ************
% 'ievery' ... ordinary array, one value per trace giving the frequency at
%               which markers are plotted. A value of 1 means that a marker is placed at every point
%               while a value of 10 means a maker is placed at every tenth point. Must be a value >=
%               1 for each trace. Has no effect if 'markers' is empty or 'none'
% ************* Default is ones(1,ntraces) ************
% 'markersizes' ... ordinary array, one value per trace giving the
%               marker size. A value of 1 is the default size while 2 is double size and .5 is half
%               size
% ************* Default is ones(1,ntraces) ************
% 'colors' ... cell array of colors, one per trace giving the
%               color. Common choices are 'b', 'r', 'g', and 'c'. A value of '' gets the
%               default color. A value of x, 0<= x <=1, is interpreted as a gray level with 0
%               being black and 1 white.
...
%               in the third dimension. This is intended to give control over how the various traces
%               overlay one another. There must be one value per trace and the default is 1:ntraces
%               which means that the traces are plotted on top of each other in the order of their
%               input. The first trace is on the bottom and the last trace is on top. For example,
%               if there are 3 traces then 1:3 gives the default order while [3 1 2] makes the first
%               trace on top and the last trace on the bottom. You can think of zorder as giving the
%               data a third dimension that is perpendicular to the plot and has the value of
%               zorder. This really only matters if 'distribute' is set to 'n'.
% ********* default 1:ntraces *********
% 'tracespacing' ... a scalar to adjust the tracespacing. A value of 1 is
%               the default. A value of 2 would space the traces 2 times farther apart. Values
%               greater than 0 and less than 5 accepted.
% ********** default is 1 ***********
% 'increment' ... spacing between traces. Defaults to the maximum absolute value found on any of the
%               traces.
% ********** default is max abs on any trace ***********
%
...
if (~iscell(times))
    ...
    for k=1:ntraces
        tmp{k}=times;
        if(length(times)~=length(traces{k}))
            disp('*************************************************************************')
            disp('when times is not a cell array, all of the traces must be the same length')
            disp('consider providing times as a cell array')
            error([' trace ' int2str(k) ' has the wrong length compared to times'])
        end
    end
    times=tmp;
end
...
%find the maximum min time and the minimum max time, make sure traces are doubles
tmaxs=zeros(1,ntraces);
tmins=tmaxs;
for k=1:ntraces
    tmins(k)=min(times{k});
    tmaxs(k)=max(times{k});
    traces{k}=double(traces{k});
end
tmax=min(tmaxs);
tmin=max(tmins);
...
if(nargin>2)
    ...
    for k=1:2:nargs
        ...
        if(strcmp(arg,'colors')||strcmp(arg,'color'))
            ...
            if(~iscell(colors))
                if(length(colors)==ntraces)
                    ...
                    error('unrecognized color array')
                end
            end
            recognized=1;
        end
        ...
        if(strcmp(arg,'tracespacing'))
            ...
            if(length(trspace)~=1)
                error('tracespacing must be a scalar');
            end
        ...
        if(strcmp(arg,'increment'))
            increment=varargin{k+1};
            if(length(increment)~=1)
                error('increment must be a scalar');
            end
            if(increment<=0)
...
%check color for graylevels
for j=1:length(colors)
    if(isnumeric(colors{j}) && length(colors{j})==1)
        if(colors{j}>=0 && colors{j}<=1)
            colors{j}=colors{j}*ones(1,3);
        else
            error(['the ' int2str(j) 'th colors entry has a bad gray level'])
        end
    end
end
...
%         am=max(abs(traces{k}));
    ...
    if(increment==0)
        ...
        if(order==0)
            ystart=0;
            inc=increment*trspace;
        else
            ystart=(ntraces-1)*increment;
            inc=-increment*trspace;
        end
    end
else
    ...
    if(strcmp(direction,'h'))
        ...
        if(~isempty(names{k}))
            if(ischar(names{k}))
                ...
                dely=inc*.1;
                if(strcmp(namesloc,'end'))
                    hn{k}(2)=text(time(end),ystart+(k-1)*inc+nameshift*inc-dely,-1,name2,...
                        'fontsize',fs,'horizontalalignment',namesalign,'backgroundcolor','w',...
                        'interpreter','none');
                    hn{k}(1)=text(time(end),ystart+(k-1)*inc+nameshift*inc+dely,-1,name1,...
                        'fontsize',fs,'horizontalalignment',namesalign,'backgroundcolor','w',...
                        'interpreter','none');                    
                elseif(strcmp(namesloc,'middle'))
                    imiddle=round(length(time)/2);
                    hn{k}(2)=text(time(imiddle),ystart+(k-1)*inc+nameshift*inc-dely,-1,name2,...
                        'fontsize',fs,'horizontalalignment',namesalign,'backgroundcolor','w',...
                        'interpreter','none');
                    hn{k}(1)=text(time(imiddle),ystart+(k-1)*inc+nameshift*inc+dely,-1,name1,...
                        'fontsize',fs,'horizontalalignment',namesalign,'backgroundcolor','w',...
                        'interpreter','none');
                else
                    hn{k}(2)=text(time(1),ystart+(k-1)*inc+nameshift*inc-dely,-1,name2,...
                        'fontsize',fs,'horizontalalignment',namesalign,'backgroundcolor','w',...
                        'interpreter','none');
                    hn{k}(1)=text(time(1),ystart+(k-1)*inc+nameshift*inc+dely,-1,name1,...
                        'fontsize',fs,'horizontalalignment',namesalign,'backgroundcolor','w',...
                        'interpreter','none');
                end
            end
        ...
        if(~isempty(names{k}))
            if(strcmp(namesloc,'end'))
                hn{k}=text(ystart+(k-1)*inc+nameshift*inc,time(end),-1,names{k},'fontsize',fs,...
                'rotation',rotation,'backgroundcolor','w',...
                        'interpreter','none');
            elseif(strcmp(namesloc,'middle'))
                imiddle=round(length(time)/2);
                hn{k}=text(ystart+(k-1)*inc+nameshift*inc,time(imiddle),-1,names{k},'fontsize',fs,...
                'rotation',rotation,'backgroundcolor','w',...
                        'interpreter','none');
            else
                hn{k}=text(ystart+(k-1)*inc+nameshift*inc,time(1),names{k},-1,'fontsize',fs,...
                'rotation',rotation,'backgroundcolor','w',...
                        'interpreter','none');
            end
        end
