...
% plotimage(smat)
%
% plotimage2 does a quick plot of a seismic matrix in a figure window
% (made by plotimage2). By default, it plots the seismic matrix in gray levels
...
%
% SCALE_OPT=1 ... the plotimage2 window begins in mean scaling mode with a clip level of 4
%         =2 ... the plotimage2 window begins in maximum scaling mode (no clipping).
...
%
% Each plotimage2 window can also be set to the status of "independent", "master", or "slave". This
...
% data while for the "slave" case the numbers are the same as for the most recent plotimage2 window
% that is declared as "master". This allows two plotimage2 windows to be displayed in true relative
...
if(XAXISTOP)
 ...
 hzoompick=uicontrol('style','popupmenu',...
	...
	'position',[x 0 wd ht],'callback','plotimage2(''zoompick'')');
 x=x+wd+sep;
 wd=.18;
 hflip=uicontrol('style','popupmenu',...
	...
	'position',[x 0 wd ht],'callback','plotimage2(''flip'')',...
	'userdata',1);
 x=x+wd+sep;
 wd=.05;
...
x=x-wd-sep;
        ...
        hslider=uicontrol('style','slider','string','Bright','units','normalized','position',...
            [x,0,wd,ht/2],'callback','plotimage2(''brighten'')',...
...
% 	'position',[x 0 wd ht/2],'callback','brighten(-.5)');
 ...
 hcmap=uicontrol('style','pushbutton','string','Colormap',...
 	'units','normalized',...
	'position',[x 0 wd ht],'callback','plotimage2(''colormap'')',...
	'visible','off');
 %x=x+wd+sep;
 wd=.17;
 ...
 hmaster=uicontrol('style','popupmenu','string','Ind.|Master|Slave',...
 	...
 	'callback','plotimage2(''rescale'')','value',ampflag);
 x=x+wd+sep;
 wd=.16;
 hscale=uicontrol('style','popupmenu','string',str2mat('Mean scaling',...
 	...
 	'callback','plotimage2(''rescale'')','value',scaleopt);
 ...
 hclip=uicontrol('style','popupmenu','string',clipmsg,...
 	...
 	'callback','plotimage2(''rescale'')','value',iclip,...
 	'visible',vis);
...
% 	set(gcf,'name','Seismic Image Plot, Simplezooming installed (Use MB1)');
 case 2
	drawlineinit('plotimage2(''pick'')',1);
