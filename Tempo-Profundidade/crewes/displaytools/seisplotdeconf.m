function datar=seisplotdeconf(seis1,t1,x1,dname1)
% SEISPLOTDECONF: Interactive frequency-domain stationary deconvolution of a seismic stack or gather
%
% datar=seisplotdeconf(seis,t,x,dname)
%
...
global DECONGATE_TOP DECONGATE_BOT DECON_FSMO DECON_STAB DECON_FMIN DECON_FMAX DECON_PHS
global NEWFIGVIS
if(~ischar(seis1))
    action='init';
else
...
%     brighten(.5);
    ...
    lw=.5;
    xs=[x1(1) x1(end)];
    h1=line(xs,ttop,'color','r','linestyle','-','buttondownfcn','seisplotdeconf(''dragline'');','tag','ttop','linewidth',lw);
    h2=line(xs,tbot,'color','r','linestyle','--','buttondownfcn','seisplotdeconf(''dragline'');','tag','tbot','linewidth',lw);
    ...
    uicontrol(hfig,'style','pushbutton','string','Use published gate','tag','setgate','units','normalized',...
        'position',[xnow ynow 1.5*wid .5*ht],'callback','seisplotdeconf(''setgate'');',...
    ...
    uicontrol(hfig,'style','pushbutton','string','Info','tag','info','units','normalized',...
        'position',[.01,.97,.5*wid,.5*ht],'callback','seisplotdeconf(''info'');',...
        'backgroundcolor','y');
    ...
    uicontrol(hfig,'style','pushbutton','string','Hide input','tag','hideshow','units','normalized',...
        'position',[xnow,ynow,wid,ht],'callback','seisplotdeconf(''hideshow'');','userdata','hide');
    %the toggle button
    ynow=ynow-ht;
    uicontrol(hfig,'style','pushbutton','string','Toggle','tag','toggle','units','normalized',...
        'position',[xnow,ynow,wid,ht],'callback','seisplotdeconf(''toggle'');','visible','off');
    ...
    ynow=ynow-ht-sep;
    xnow=xnow+sep;
    uicontrol(hfig,'style','text','string','Deconf parameters:','units','normalized',...
    ...
    uicontrol(hfig,'style','text','string','fsmo:','units','normalized',...
        'position',[xnow,ynow,wid,ht],'tooltipstring','frequency domain smoother in Hz');
    uicontrol(hfig,'style','edit','string',num2str(fsmo),'units','normalized','tag','fsmo',...
        'position',[xnow+wid+sep,ynow,wid,ht],'tooltipstring','Enter a value in Hz between 0 and 100');
    ynow=ynow-ht-sep;
    ...
    uicontrol(hfig,'style','text','string','phase:','units','normalized',...
        'position',[xnow,ynow,wid,ht],'tooltipstring','phase of decon operator');
    uicontrol(hfig,'style','popupmenu','string',{'zero','minimum'},'units','normalized','tag','phsd',...
        'position',[xnow+wid+sep,ynow,1.5*wid,ht],'tooltipstring','choose zero or minimum','value',phsd+1);
    ynow=ynow-ht-sep;
    uicontrol(hfig,'style','pushbutton','string','Apply Deconf and Filter','units','normalized',...
        'position',[xnow,ynow,3*wid,ht],'callback','seisplotdeconf(''applydecon'');',...
        'tooltipstring','Apply current deconf and filter specs','tag','deconbutton',...
        'backgroundcolor','y');
...
%     wid=wid/3;
    ...
    hbg1=uibuttongroup('position',[xnow,ynow,widbg,3*ht],'title','Filter type','tag','choices',...
        'selectionchangedfcn','seisplotdeconf(''filterchoice'');');
...
%     wid2=.2;
    ...
    uicontrol(hfig,'style','pushbutton','string','Apply Filter Only','units','normalized',...
        'position',[xnow,ynow,1.5*wid,ht],'callback','seisplotdeconf(''applyfilter'');',...
    ...
    uicontrol(hfig,'style','pushbutton','string','Show spectra','units','normalized',...
        'position',[xnow,ynow,wid,ht],'callback','seisplotdeconf(''spectra'');',...
    ...
    uicontrol(hfig,'style','pushbutton','string','Zoom #1 like #2','units','normalized',...
        'position',[xnow ynow wid ht],'tag','1like2','callback','seisplotdeconf(''equalzoom'');');
    
    pos=get(hax2,'position');
    xnow=pos(1)+.5*pos(3)-.5*wid;
    uicontrol(hfig,'style','pushbutton','string','Zoom #2 like #1','units','normalized',...
        'position',[xnow ynow wid ht],'tag','2like1','callback','seisplotdeconf(''equalzoom'');');
    ...
    uicontrol(hfig,'style','popupmenu','string','Diddley','units','normalized','tag','results',...
        'position',[xnow,ynow,wid,ht],'callback','seisplotdeconf(''select'');','fontsize',fs,...
        'fontweight','bold','uicontextmenu',hcm)
    uicontrol(hfig,'style','popupmenu','string','Diddley','units','normalized','tag','shortresults',...
        'position',[xnow+.25*wid,ynow,.5*wid,ht],'callback','seisplotdeconf(''select'');','fontsize',fs,...
    ...
    uicontrol(hfig,'style','pushbutton','string','Delete this result','units','normalized',...
        'tag','delete','position',[xnow,ynow+1.75*ht,wid,ht],'callback','seisplotdeconf(''delete'');',...
        'tooltipstring','Delete this result (no undo)','userdata',1);
    
    %shortnames
    uicontrol(hfig,'style','radiobutton','string','Short Names','units','normalized',...
        'tag','shortnames','position',[xnow,ynow+2.75*ht,wid,ht],'callback','seisplotdeconf(''shortnames'');',...
...
%     whitefig;
    ...
    set(hax2,'tag','seis2');
    seisplotdeconf('applydecon');
%     if(iscell(dname2))
%         dn2=dname2{1};
%     else
%         dn2=dname2;
%     end
    set(hfig,'name',['Spiking decon (frequency) analysis for ' dname1],'closerequestfcn','seisplotdeconf(''close'');',...
...
%     idesign=near(t,ttop,tbot);
    ...
    if(fsmo<0 || fsmo>100)
        msgbox('fsmo is unreasonable, enter a value in Hz');
        return;
    end
    %get the stab 
    ...
    %get the phsd 
    hphsd=findobj(gcf,'tag','phsd');
    phsd=get(hphsd,'value')-1;
    
    %deconvolve
    t1=clock;
    seisd=deconf_stack(seis,t,0,ttop,tbot,fsmo,stab,phsd);
    DECONGATE_TOP=ttop;
    DECONGATE_BOT=tbot;
    DECON_FSMO=fsmo;
    DECON_STAB=stab;
    ...
    set(hdbut,'userdata',{seisd,ttop,tbot,fsmo,stab,phsd});
    seisplotdeconf('applyfilter');
...
elseif(strcmp(action,'applyfilter'))
    ...
    switch choice
        case 'Stationary'
            seisplotdeconf('applyfilterstat');
        case 'Time variant'
            seisplotdeconf('applyfiltertv');
    end
...
%     set(hi,'cdata',seis2);
    ...
    dname=['DeconF fsmo=' num2str(fsmo) ', stab=' num2str(stab) ', phsd=' num2str(phsd) ', gate ' time2str(ttop) '-' time2str(tbot)];
    ...
    if(isempty(results))
         nresults=1;
        results.names={name};
        results.shortnames={'Frequency Decon #1'};
        results.expnumber=1;
        results.data={seis2};
        ...
        results.shortnames{nresults}=['Frequency Decon #' int2str(results.expnumber(nresults))];
...
elseif(strcmp(action,'spectra'))
    ...
    t=get(hi,'ydata');
    name1='Input';
    name2='Decon (frequecy domain)';
    hspec=findobj(hmaster,'tag','spectra');
    hspecwin=get(hspec,'userdata');
    CRcallback='seisplotdeconf(''closespec'');';
    Winname=[];
...
%     iprev=get(hdelete,'userdata');
    ...
    hphs=findobj(hfig,'tag','phsd');
    set(hphs,'value',results.phsd{iresult}+1);
    ...
    set(hdbut,'userdata',{results.datanf{iresult},results.ttop{iresult},results.tbot{iresult},...
        results.fsmo{iresult},results.stab{iresult}});
    %update clipping
...
elseif(strcmp(action,'info'))
    ...
    msg1={'Deconvolution',{['Deconvolution refers to the step in seismic data processing intended ',...
        ...
        ['Other than the design window, the two major parameter choices are the operator length "fsmo" and ',...
    ...
    msg3={'Parameters',{['For the deconvolution there are only three things to specify but this tool ',...
        ...
        '"fsmo" and "stab". If you are new to deconvolution you should read the information ',...
        ...
        'window should be no smaller that N*fsmo where N is between 3 and 6 depending on your ',...
        '"thumb". :-) You are encouraged to violate this rule and see what happens. '], ' ',...
        ['The parameter "fsmo" refers to the length of the deconvolution operator in time and is ',...
        ...
        'suppressed than you desire after deconvolution, then you might try making fsmo larger or ',...
        'stab smaller. But before you do so, be sure that the high frequency suppression is not ',...
        'caused by your filter. Practitioners typically use values for fsmo in the range 0.04<fsmo<0.5 ',...
        'and usually change values by a factor of 2. So, if your fsmo is 0.08 and you want more ',...
