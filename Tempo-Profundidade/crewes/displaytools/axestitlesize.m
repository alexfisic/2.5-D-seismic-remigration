function axestitlesize(factor,hdaddy)
% AXESTITLESIZE .... chane the size of axes titles
%
% axestitlesize(factor,hdaddy)
...
for k=1:length(haxes)
    hlt=get(haxes(k),'title');
    if(~isempty(hlt))
        set(hlt,'fontsize',factor*get(hlt,'fontsize'));
    end
end