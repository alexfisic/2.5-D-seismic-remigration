...
%
% Publishing color figures for simple line plots can be expensive and is often not necessary.
% Instead of using color, the lines can be plotted with different shades of gray. In addition
% the line width and line style can be varied. LINESGRAY is a utility that makes this easier.
% Each input argument is a cell array with up to 7 entries specifying a line to be plotted.
% There can be any number of lines. The identity of each entry in a line specification is
% determined by it's order. The order of entries in each line spec (cell array) is (note the
% syntax): {x-coordinates, y-coordinates, linestyle, linewidth, gray-level, markertype,
% markersize} So there are at most 7 entries and there must be at least the first two. The
% lines are drawn with the fundamental drawing function "line" so they are added to the current
% figure rather than clearing it as "plot" does.
%
