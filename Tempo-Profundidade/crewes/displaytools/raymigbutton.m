...
if(strcmp(action,'init'))
    ...
    if(isempty(msg))
        label=['Migrate fig' int2str(figno1) ' -> fig' int2str(figno2)];
        width=.15;
    else
        label=['Migrate fig' int2str(figno1) ' -> fig' int2str(figno2) ' ' msg];
        width=.2;
    end
    hbutt=uicontrol(figseis,'style','pushbutton','string',label,'units','normalized',...
        'position',[startpos .95 width .05],'callback','raymigbutton(''migrate'')',...
