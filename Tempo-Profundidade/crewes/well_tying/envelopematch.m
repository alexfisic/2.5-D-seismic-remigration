function [sonout,picks]=envelopematch(trcs,t,son,rho,z,wlet,tw)
% ENVELOPEMATCH is a gui interface that will allow the user to alter a sonic
% log to match Hilbert envelopes.
%
%[sonout,picks]=envelopematch(trcs,t,son,rho,z,wlet,tw)
%
% Variables In
...
%% initiate gui
h.Ffig=figure('units','pixels','position',[150 150 1000 700],'name','Stretch Sonic Logs','NumberTitle','off','CloseRequestFcn',@close);
...
h.Btpic=uicontrol('units','normalized','position',[.1 .40 .2 .05],'string','TopPick','style','pushbutton','callback',@pick,'userdata','tpick');
h.Bwpic=uicontrol('units','normalized','position',[.1 .35 .2 .05],'string','Well Envelope Pick','style','pushbutton','callback',@pick,'userdata','wpick');
h.Bspic=uicontrol('units','normalized','position',[.1 .30 .2 .05],'string','Trace Envelope Pick','style','pushbutton','callback',@pick,'userdata','spick');
...
h.Vtrc=mean(trcs,2);
h.Vtrcenv=sqrt(h.Vtrc(:).^2+phsrot(h.Vtrc(:),90).^2);
h.Vt=t;
h.Vson=son;
h.Voson=son;
h.Vlson=son;
...
uiwait(h.Ffig);
h=guidata(h.Ffig);
[sg,tlog,r,pm,p,tz,zt]=seismotz(h.Voson,h.Vrho,h.Vz,h.Vson,0,0,h.Vwlet,h.Vtw);
sgram=zeros(size(t));rcs=sgram;
sgram(1:near(h.Vt,tlog(end)))=sg(1:near(h.Vt,tlog(end)));
rcs(1:near(h.Vt,tlog(end)))=r(1:near(h.Vt,tlog(end)));
sonout=h.Vson;
picks=h.picks;
delete(h.Ffig)
end
...
h=guidata(hobj);
[sgram,tlog,rcs,pm,p]=seismotz(h.Voson,h.Vrho,h.Vz,h.Vson,0,0,h.Vwlet,h.Vtw);
h.Vsyn=zeros(size(h.Vt));
h.Vsyn(1:near(h.Vt,tlog(end)))=sgram(1:near(h.Vt,tlog(end)));
h.Vsyn(1:25)=0;
h.Vsynenv=sqrt(h.Vsyn(:).^2+phsrot(h.Vsyn(:),90).^2);
h.Vsynenv=h.Vsynenv./max(h.Vsynenv);
h.Vtrcenv=h.Vtrcenv./max(h.Vtrcenv(1:near(h.Vt,tlog(end))));
h.Vsyn=h.Vsyn./max(h.Vsyn);
h.Vmpwr=h.Vt(find(cumsum(h.Vsyn.^2)>.99*max(cumsum(h.Vsyn.^2)),1));
set(h.Ffig,'currentaxes',h.Amaxis);
        wtva(h.Vtrcenv+.25,h.Vt,[.5 0 0],h.Fwtva+.25);
        wtva(h.Vsynenv+1.25,h.Vt,[0 0 .5],h.Fwtva+1.25);
...
flipy;
set(gca,'ylim',[0 h.Vmpwr],'xlim',[0 2.5]);
guidata(hobj,h);
pview(hobj,u);
end
...
if h.Amaxis==get(h.Ffig,'currentaxes')
    ...
    wtva(h.Vtrcenv+.25,h.Vt,[.5 0 0],h.Fwtva+.25);
        wtva(h.Vsynenv+1.25,h.Vt,[0 0 .5],h.Fwtva+1.25);
    plot([0 2.5],[h.Vtpic h.Vtpic],'k:','linewidth',3);
    plot([1.25  2.25],[h.Vwpic h.Vwpic],'k:','linewidth',3);
    plot([.25  1],[h.Vspic h.Vspic],'k:','linewidth',3);
    title('Hilbert Envelopes');
    ylabel('Time (s)');
    l=legend('Seismic','','Synthetic','','orientation','horizontal');
    set(l,'units','normalized','position',[0.60 0.015 0.35 0.04]);
...
h=guidata(hobj);
if strcmp(get(hobj,'userdata'),'tpick')
    set(h.Btpic,'string','Top Envelope Pick','callback',@pick);
    set(h.Bwpic,'enable','on');
    set(h.Bspic,'enable','on');
end
if strcmp(get(hobj,'userdata'),'wpick')
    set(h.Bwpic,'string','Well Envelope Pick','callback',@pick);
    set(h.Btpic,'enable','on');
    set(h.Bspic,'enable','on');
end
if strcmp(get(hobj,'userdata'),'spick')
    set(h.Bspic,'string','Trace Envelope Pick','callback',@pick);
    set(h.Bwpic,'enable','on');
    set(h.Btpic,'enable','on');
end
...
plot([0 2.5],[h.Vtpic h.Vtpic],'k:','linewidth',3);
plot([1.25  2.25],[h.Vspic h.Vspic],'k:','linewidth',3);
plot([.25 1],[h.Vspic h.Vspic],'k:','linewidth',3);
ylabel('Time (s)');
title('Hilbert Envelopes');
l=legend('Seismic','','Synthetic','','orientation','horizontal');
