function [theo,tlog,rcs,pm,p]=seismogram(vins,dens,z,wlet,tw,tzobj,x,fmult,fpress,tmin,tmax)
...
rcs=imp2rcs(imptime);
if(isnan(tmin))
    tmin=round(tlog(1)/dtout)*dtout;
end
if(isnan(tmax))
    tmax=round(tlog(end)/dtout)*dtout;
end
if(tlog(1)>tmin)
    npad=round((tlog(1)-tmin)/dtout);
%     tnew=(tmin:dtout:tlog(1)-dtout)';
    rnew=zeros(npad,1);
%     tlog=[tnew;tlog];
    rcs=[rnew;rcs];
end
if(tlog(end)<tmax)
    npad=round((tmax-tlog(end))/dtout);
%     tnew=(tlog(end)+dtout:dtout:tmax)';
    rnew=zeros(npad,1);
%     tlog=[tlog;tnew];
    rcs=[rcs;rnew];
end
tlog=dtout*(0:length(rcs)-1)';
