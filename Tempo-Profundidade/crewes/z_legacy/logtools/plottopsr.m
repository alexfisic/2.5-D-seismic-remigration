function htops=plottopsr(tops,topnames,klr)
% PLOTTOPSR: plot tops on a log plot with the labels on the right
%
% htops=plottopsr(tops,topnames,klr)
%
% PLOTTOPSR will plot horizontal lines across the current figure
% for each of a set of formation tops. The tops will be labeled only on the
% right and will extend for the length of the x axis. It is assumed
% that the log is plotted with the y coordinate being depth
% (or time). licking on any top will cause it's name to be displayed.
