function htops=plottopsc(tops,topnames,klr)
% PLOTTOPSC: plot tops on a log plot with no labels
%
% htops=plottopsc(tops,topnames,klr)
%
% PLOTTOPSC will plot horizontal lines across the current figure
% for each of a set of formation tops. The tops will be not be labeled
...
% PLOTTOPSC is intended for use in a subplot where tops names are not
% wanted because they are already named in adjacent subplots.
% 
...
%  polylog=polyval(c,ylog);
 ntops=length(tops);
	...
	for k=1:ntops
        ...
        htops(ntops+k)=text(min(xlim(2)),tops(k),'',...
