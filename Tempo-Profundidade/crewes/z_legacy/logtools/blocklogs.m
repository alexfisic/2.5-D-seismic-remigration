function [vpblk,vsblk,rhoblk,zout,ztop]=blocklogs(arg1,dzblk,dzout,vp0,vs0,rho0,mud1,mud2,gardner1,gardner2)
% BLOCKLOGS: reads an las file for sonic and density logs and blocks them
% 
% [vpblk,vsblk,rhoblk,zblk,ztop]=blocklogs(arg1,dzblk,dzout,vp0,vs0,rho0,mud1,mud2,gardner1,gardner2)
%
% This function reads an LAS well file (or accepts a log structure) and returns vectors of vp, vs, and
% rho (density) which have been averaged over the interval dzout. A gradient overburden is attached
% that extends from specified surface (z=0) values of vp, vs, and rho, to the local averages of
% values found at the tops of the logs. This function always averages slowness and then inverts this
% to get velocity.
%
% arg1 ... either a string containing the LAS file name (including path) or a structure with the
%       following fields:
%       arg1.depth = depth vector
%       arg1.sp = p-wave sonic
%       arg1.ss = s-wave sonic
%       arg1.rho = density
%       arg1.units = string, either 'm' or 'ft'
%   The first two fields are mandatory and must be the same length. No nans allowed. If ss and rho
%   are not empty then they must also be the same length as z. If empty then they will be synthesize
%   from Mudrock and Gardner
%
% If arg1 is a string, then it is the LAS file name.
%
...
if(ischar(arg1))
    ...
    z=arg1.z;
    sp=arg1.sp;
    if(length(z)~=length(sp))
        error('lengths of z and sp must be the same')
    end
    ipson=2;
    ss=arg1.ss;
    if(~isempty(ss))
        if(length(z)~=length(ss))
            error('lengths of z and ss must be the same')
        end
        isson=3;
    else
        ss=zeros(size(sp));
    ...
    rho=arg1.rho;
    if(~isempty(rho))
        if(length(z)~=length(rho))
            error('lengths of z and rho must be the same')
        end
        iden=4;
    else
        rho=zeros(size(sp));
