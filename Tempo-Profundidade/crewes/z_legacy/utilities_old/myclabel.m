function clabel(cs, contours,labelcolor)
...
if manual
   ...
   % next two code lines added by Margrave, November 1993
   % these may be flipped if we are looking at a flipped image plot
...
% Label the point.
   if okay
      s = [' ' sprintf('%0.3g', c)];
      code = ['+' labelcolor];
	line(xx,yy,'color',labelcolor,'marker','o','erasemode','none');
     % plot(xx, yy, code);
	text(xx, yy, s, 'verticalalignment', 'bottom', 'horizontalalignment', 'left',...
			'erasemode','none','color',labelcolor);
   end
end
