...
% 
%
% NOTE: Scaling options are controlled by defining a global variable in your
% base workspace and setting its value as needed. For example, at the matlab
% prompt, type:
% >> global scaleopt
% This defines the globale variable called "scaleopt". Plotimage recognizes the
%	following values for scaleopt:
%   scaleopt=1 ... The mean and standard deviation of all samples are computed.
...
%   scaleopt=2 ... The maximum absolute value of the data is computed (say mxs).
...
% It is suggested that mean scaling (scaleopt=1) be used for real data (this is the
%	default) and max scaling (scaleopt=2) be used for synthetic data. 
%
...
%
global scaleopt
if ( isempty(scaleopt) ) scaleopt=1; end %default to mean scaling
if( nargin < 1 | ~ischar(smat) )
	action='init';
else
	action = smat;
end
