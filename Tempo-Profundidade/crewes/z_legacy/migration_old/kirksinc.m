function [arymig,tmig,xmig]=kirksinc(aryin,aryvel,dt,dx,params)
% [arymig,tmig,xmig]=kirksinc(aryin,aryvel,t,x,params)
%
% KIRKSINC is a simple post stack Kirchhoff time migration routine
%	that uses sinc function sample interpolation.
%
...
%               it is assumed to give the rms velocity for each sample 
%               location.
...
for ktr=trtarget
	...
	for kaper=1:length(truse)
		...
		tx=sqrt( offset2(kaper)./v2(samptarget) + t2(samptarget) );
		
		tmp = zeros(1,length(samptarget));
		tmp = sinci(aryin(:,truse(kaper)),tpad,tx);
