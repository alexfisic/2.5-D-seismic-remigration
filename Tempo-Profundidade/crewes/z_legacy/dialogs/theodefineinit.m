...
%		holealg,theopt,a,m,msg,sonicnum,densitynum,name)
%
% THEODEFINEINIT is called by LOGSEC to initiate a dialog  to define the
...
%		1= use Gardners relation exclusivly; 2=use density section where 
%		defined, fill in holes with Garnders relation after hol filling on
%		sonic section 3) use density section exclusively. Fill holes on it
...
% theopt ... flag giving the default for the multiple option
%		1=primaries only, 2=primaries plus multiples
...
set(hax,'userdata',[abs(transfer) nan abs(sections) nan abs(msg) nan...
	hmasterfig densopts holealg theopt a m sonicnum densitynum abs(name)]);
theodefine('init')