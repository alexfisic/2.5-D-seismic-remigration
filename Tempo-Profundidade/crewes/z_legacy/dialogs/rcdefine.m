...
if( strcmp(action,'init'))
	...
	htitle=uicontrol('style','text','position',[xnow,ynow,width,height],'string',...
		'Reflection Coefficient Computation Specifications','foregroundcolor','r');
	xnow=sep;
	...
	hsonicsec=uicontrol('style','popupmenu','string',sections,'position',...
		[xnow,ynow,width,height],'callback','rcdefine(''setsection'')',...
	...
	hdensopt=uicontrol('style','popupmenu','string',doptmsg,...
		'position',[xnow,ynow,width,height],'callback','rcdefine(''densopt'')',...
...
%	ynow=ynow-height-sep;
%	width=150;
	hdomainlbl=uicontrol('style','text','string','Computation Domain:','position',...
		[xnow,ynow,width,height],'visible','off');
%	xnow=xnow+width+sep;	
%	width=150;
	hdomain=uicontrol('style','popupmenu','string','Time|Depth',...
		'position',[xnow,ynow,width,height]','value',domain,'visible','off');
 	xnow=sep;
	ynow=ynow-height-sep;
 	width=400;
 ...
 width=40;
	...
	ynow=ynow-height-sep;
	width=100;
	hnamelbl=uicontrol('style','text','string','Name the rcs:','Position',...
		[xnow,ynow,width,height]);
		
	xnow=xnow+width+sep;
	width=300;
	ind=findstr(sections,'|');
	if(isempty(ind)) ind=length(sections)+1; end
	if(length(name)==1)
		name=['rc: ' sections(1:ind(1)-1)];
	else
		name=setstr(name);
	end
	...
	hdone=uicontrol('style','pushbutton','string','Done','position',...
		[xnow,ynow,width,height],'callback','rcdefine(''done'');',...
		'userdata',transfer,'enable','on');
	xnow=xnow+width+sep;
 	width=60;
	hcancel=uicontrol('style','pushbutton','string','Cancel','position',...
		[xnow,ynow,width,height],'callback','rcdefine(''cancel'');');
	set(hfig,'userdata',[hmasterfig,hmsg,hsonicsec,hdensitysec,hdensopt,...
		hholealg,hdomain,hgardnerlbl,halbl,ha,hmlbl,hm,hdone,hcancel hname]);
...
if(strcmp(action,'getparameters'))
	...
	%the domain
	domain=get(hdomain,'value');
	
	%store this stuff
	
	set(hax,'userdata',[sonicnum densitynum densopt holealg domain a m abs(name)]);
...
if(strcmp(action,'setsection'))
	...
	set(hname,'string',['rc: ' secnames(secnum,:)]);
