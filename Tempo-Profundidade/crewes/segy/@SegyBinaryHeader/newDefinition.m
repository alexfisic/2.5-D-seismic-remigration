function [bd, segyrev] = newDefinition(obj, segyrev)
%Create a new binary header definition based on the SEG-Y revision number
%
% function [bd, segyrev] = newDefinition(obj, segyrev)
%
...
if ~isequal(segyrev,0) && ~isequal(segyrev,1) && ~isequal(segyrev,2)
    mm_warndlg('@BinaryHeader/newDefinition: Unknown SEG-Y revision number, trying Rev. 1','Warning!',obj.GUI);
    segyrev = 1;
end
...
elseif segyrev == 2
    ...
    mm_errordlg('@BinaryHeader/newDefinition: Code should never have reached this point','Error!',obj.GUI);
end
...
function ca = mkunassigned(widx1,widx2,sbyte)
    ...
    ca = {sn{1:end-1};dt{:};sb{:};ln{1:end-1}}';
