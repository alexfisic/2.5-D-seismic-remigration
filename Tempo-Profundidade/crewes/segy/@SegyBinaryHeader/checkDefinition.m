...
%Check a binary header definition cell array for validity
%
% function checkDefinition(obj, bd)
%
...
if ~iscell(bd)
    mm_errordlg('@BinaryHeader/checkDefinition: Definition must be a cell array','Error!',obj.GUI);
end
...
if ~isequal(ncols,4)
    mm_errordlg('@BinaryHeader/checkDefinition: Definition must have 4 columns','Error!',obj.GUI);
end

%make sure columns contain expected data types
if ~iscellstr(bd(:,1:2)) || ~isnumeric([bd{:,3}]) || ~iscellstr(bd(:,4))
    mm_errordlg('@BinaryHeader/checkDefinition: Definition cols 1,2,4 must be ''char'', col 3 must be ''numeric''','Error!',obj.GUI);
end

%check to see if column1 contains duplicate field names
nfieldnames = length(unique(bd(:,1)));
if ~isequal(nfieldnames,nfields)
    mm_errordlg('@BinaryHeader/checkDefinition: ''Short Name'' column contains duplicate field names ','Error!',obj.GUI);
end
...
if sum(vt) ~= nfields
    ...
    mm_errordlg(['@BinaryHeader/checkDefinition: ''Data Type'' column  has invalid datatype(s) at row(s) '...
        br(1:end-1) ],'Error!',obj.GUI);
end
...
if ~isequal(obj.HDRSIZE,sum(bytes))
        mm_warndlg(['Total number of bytes represented by ''Data Type'' column ~= ' ...
            num2str(obj.HDRSIZE)],'Warning!',obj.GUI);            
end    
...
if sum(vt)
    br = num2str(rownums(vt),'%d, ');
    mm_errordlg(['@BinaryHeader/checkDefinition: ''Start Byte'' column has inconsistent startbyte at row(s) '...
        br(1:end-1) ],'Error!',obj.GUI);
end
