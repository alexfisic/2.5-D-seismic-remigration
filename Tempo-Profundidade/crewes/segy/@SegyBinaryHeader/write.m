...
if ~isstruct(bh)
    mm_errordlg('@BinaryHeader/write: Header to write to disk must be a struct','Error!',obj.GUI);
end

%check bh struct
obj.check(bh);

%update format code if there is an override
bh.(obj.byte2word(25)) = obj.FormatCode;

%make sure text header has already been written
fs = obj.fsize();
if isequal(fs,0)
    mm_errordlg('@BinaryHeader/write: File on disk does not contain a text file header','Error!',obj.GUI);
elseif fs >0 && fs <obj.OFFSET
    mm_errordlg(['@BinaryHeader/write: File on disk text header is not ' num2str(obj.OFFSET) ' bytes'],'Error!',obj.GUI);
elseif fs >obj.OFFSET
    mm_errordlg('@BinaryHeader/write: Refusing to overwrite existing binary file header','Error!',obj.GUI);
end
...
try
    ...
    mm_errordlg(['@BinaryHeader/write: ' ex.message],'Error!',obj.GUI);
end
