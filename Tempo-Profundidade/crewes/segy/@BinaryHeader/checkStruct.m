...
if ~isstruct(bh)
    error('@BinaryHeader/checkStruct: object to check must be a struct')
end
...
for ii = 1:ndefs
    ...
    if ~isfield(bh,fieldname)
        error(['@BinaryHeader/checkStruct: struct must contain fieldname: ' ...
            fieldname])
    else
        nfields = nfields+1;
    end
    ...
    if ~isscalar(bh.(obj.HdrFieldNames{ii}))
        error(['@BinaryHeader/checkStruct: value for fieldname: ' ...
            fieldname ...
            ' must be scalar'])
    end
