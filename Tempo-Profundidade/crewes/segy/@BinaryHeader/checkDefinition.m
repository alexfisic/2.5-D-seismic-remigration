...
if ~isequal(nfieldnames,nfields)
    error('@BinaryHeader/checkDefinition: Definition col 1 contains duplicate field names ')
end

%check to see if column1 field names have spaces in them
c = cellfun(@(X) sum(isspace(X)), bd(:,1),'UniformOutput',false);
if sum(cell2mat(c))
   error('@BinaryHeader/checkDefinition: Definition col 1 contains field names with spaces in them')
end
...
if sum(vt) ~= nfields
    ...
    error(['@BinaryHeader/checkDefinition: Definition col 2  has invalid datatype(s) at row(s) '...
        br(1:end-1) ])
end
...
if ~isequal(obj.HDRSIZE,sum(bytes))
    if isempty(obj.GUI) || isa(obj.GUI,'handle')
        mm_warndlg(['Total number of bytes represented by Definition col 2 ~= ' ...
            num2str(obj.HDRSIZE)],'Warning!',gcbf);
    elseif objGUI
        warning(['Total number of bytes represented by Definition col 2 ~= ' ...
            num2str(obj.HDRSIZE)])
    end             
end    
...
if sum(vt)
    br = num2str(rownums(vt),'%d, ');
    error(['@BinaryHeader/checkDefinition: Definition col 3 has inconsistent startbyte at row(s) '...
        br(1:end-1) ])
end
