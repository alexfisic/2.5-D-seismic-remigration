...
%get(obj,'Position')
% disp('in guessFormatCode');

fc = BinaryHeader.readFormatCode(obj.FileID);
obj.FormatCode = fc;
obj.TrcDatFormatCode = fc;

if isequal(fc,1) %Format code 1 is ambiguous, could be IBM or IEEE floats
    %fseek to start of first trace
    File.seek(obj.FileID, obj.TraceOneOffset+obj.HDRSIZE, 'bof');
    %fread first trace data as uint32
    trcin     = File.read(obj.FileID, obj.SamplesPerTrace, 'uint32=>uint32');
