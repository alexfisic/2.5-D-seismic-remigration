...
%       or by Trace.read
% gui (optional) = 0 (no progress bar), 1 (text progress bar; 
%                    default), 2 (gui progress bar)
%
...
fs = File.size(obj.FileID);

if isequal(fs,0)
    error('@Trace/write: File is too small to contain any SEG-Y file headers');
elseif fs >0 && fs <obj.TraceOneOffset
    error(['@Trace/write: File headers are less than ' num2str(obj.TraceOneOffset) ' bytes']);
end
...
th = obj.applyCoordinateScalars(th); %apply scalars %NEEDS MORE TESTING

%% Make certain number of data samples per trace in trace headers matches binary header
%read number of samples per trace from existing binary file header on disk
nbhdrsamp = obj.SamplesPerTrace;
...
%overwrite number of samples in this trace in trace headers matches binary header
fieldname = obj.byte2word(114); %stored in byte 115; SEG-Y standard
th.(fieldname)(1:end) = nbhdrsamp; 
...
bhdrdt = BinaryHeader.readSampleInterval(obj.FileID); %read from bin hdr already written to disk
...
th.(fieldname) = 1:ndattrc +obj.TracesInFile;

%% Final check of trace headers and trace data to write
obj.check(th);
obj.check(td);

%deal with IBM floating point data
if obj.FormatCode == 1 && obj.TrcDatFormatCode == 1
    datatype = 'uint32';
    td = num2ibm(td);
else
    datatype = obj.FormatCodeType;
end
...
for jj = 1:ndattrc
    ...
    for ii = 1:length(obj.HdrDef)
        ...
        File.write(obj.FileID,v,datafmt);
	...
	File.write(obj.FileID,td(:,jj),datatype);
