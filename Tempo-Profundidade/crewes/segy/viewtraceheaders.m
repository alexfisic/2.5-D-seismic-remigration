...
%
% viewtraceheaders(tracehdrs,segyrev,msg) %simple call, won't work if your headers have anything nonstandard
% viewtraceheaders(tracehdrs,hdrdef,msg) %second argument is a header definition (modified). use when above fails
...
if(strcmp(action,'init'))
    ...
    for k=1:nfields
        ...
        data{k,2}=byte;
        data{k,4}=' ';%this is just a spacer
        data{k,5}=class(tracehdrs.(fn{k})(ktrace));
    ...
    xn=0;
    fudge=.96;
    cw=figwid*panwid*fudge;
    colwids={.35*cw, .1*cw, .3*cw,.05*cw, .2*cw};
    
    data=filldata(ktrace);
    
    ht=uitable(hdp,'data',data,'ColumnName',{'Name','Byte','Value',' ','Format'},'units','normalized',...
...
val=get(src,'value');
p=get(arg1,'position');
set(arg1,'position',[p(1) -2*val p(3:4)]);