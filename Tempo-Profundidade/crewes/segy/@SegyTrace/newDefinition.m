function [td, segyrev] = newDefinition(obj,segyrev,numexthdr)
%Returns a new trace header definition cell array based on SegyRevision
%
% function [td,segyrev] = newDefinition(segyrev,numexthdr)
%
...
if nargin < 3 || isempty(numexthdr)
    numexthdr = obj.NumExtTrcHdrs; %quietly ignored if not segyrev 2
end

if ~isequal(segyrev,0) && ~isequal(segyrev,1) && ~isequal(segyrev,2)
    mm_warndlg('@Trace/newDefinition: Unknown SEG-Y revision number, trying Rev. 1','Warning!',obj.GUI);
    segyrev = 1;
end
...
elseif segyrev == 1 || segyrev == 2    
    ...
    elseif segyrev == 2
        ...
        elseif numexthdr >1
            mm_errordlg('@Trace/newDefinition: Code does not support more than 1 extended trace headers','Error!',obj.GUI);
        end
    end  
else
    mm_errordlg('@Trace/newDefinition: Code should never have reached this point','Error!',obj.GUI);
end
...
function ca = mkunassigned(widx1,widx2,sbyte)
    ...
    sn = strsplit(sprintf('Unassigned%02d,',widx1:widx2),','); %shortname
    dt = repmat({'uint32'},widx2-widx1+1,1); %datatype
    sb = num2cell(sbyte+4*((widx1:widx2)-widx1)); %startbyte
    sc = repmat({''},widx2-widx1+1,1); %scalar header word to apply
    ln = strsplit(sprintf('Unassigned %d,',widx1:widx2),','); %longname
    
    ca = {sn{1:end-1};dt{:};sb{:};sc{:};ln{1:end-1}}';
