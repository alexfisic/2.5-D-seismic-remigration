...
%
% Algorithm: If SEG-Y FormatCode = 1 read the ten random traces from the file as
...
% actually be IEEE floating point, or FormatCode = 5. This is typically an
% issue seen with SEG-Y revision 0 files, before FormatCode = 5 became part
% of the SEG-Y standard in revision 1.
%
...
if isequal(obj.FormatCode,1)
    ...
    if obj.SamplesPerTrace <1
       mm_errordlg(['@SegyTrace: Samples per trace is: ',...
        num2str(obj.SamplesPerTrace),...
        '. Please read the help and override nsamp!'],...
        'Error!',obj.GUI);
    end
    ...
    if obj.TracesInFile > 2*ntraces+1
        ...
        ntraces = obj.TracesInFile;
        obj.fseek(obj.TOTHDRSIZE,'bof') %skip to first trace data
        try
        trcin = obj.fread([obj.SamplesPerTrace ntraces],...
            sprintf('%d*uint32',double(obj.SamplesPerTrace)), ...
            'uint32', ...
           ...
           mm_errordlg(['@Trace/guessFormatCode: Data Sample Format Code = 1 (4-byte IBM) ' ... 
               'and a random trace read from the file has failed: Are you certain there are ' ...
               sprintf('%d',obj.SamplesPerTrace) ' samples per trace?'],'Error!',obj.GUI);           
        end
    end
