...
%
% Usage: 
%   trc = SegyTrace(filename,permission,segyrevision,sampint,nsamp, ...
%                   fmtcode,byteorder,gui)
%
% Where:
...
%   sampint      = sample rate in seconds
...
methods
    function obj = SegyTrace(filename,permission,segyrevision,sampint,nsamp,fmtcode,byteorder,gui)
        ...
        %binary header on disk and copy file properties into object
        if obj.FileID > 0 && obj.fsize > 3600
            p = obj.Permission;
            obj.Permission = 'r';
            bh = SegyBinaryHeader(obj.FileID,permission,byteorder,segyrevision,gui);
...
%                     +3200*double(bh.readNumExtTxtHeaders());
        ...
        %Override number of samples per trace
        if ~isempty(nsamp)
    ...
    function set.NumExtTrcHdrs(obj, v)
        if isempty(v) || (isscalar(v) && isnumeric(v) && v>-1)
            ...
            mm_errordlg('@SegyTrace: NumExtTrcHdrs must be numeric, scalar and greater than -1',...
                'Error!',obj.GUI);
        end
    end
        ...
        switch(obj.FormatCode)
            case(1) %IBM float
                ...
                if crmemfree
                   ...
                   trcperblock = 1.0737e+09/obj.TraceSize; %hardwired 1Gb !
                end
            ...
            case(7) %int24                                
                ...
                sampinblock = trcperblock*nsamp*bpsamp;
                
                utrcdat = trcdat;
                trcdat  = zeros(nsamp,numtraces,'int32');
                
                ii = 1:trcperblock:numtraces;
                jj = 1:sampinblock:sampinfile;
                for kk = 1:length(ii)
                    traceone = ii(kk);
                    sampone = jj(kk);
                    tracetwo = ii(kk)+trcperblock-1;
