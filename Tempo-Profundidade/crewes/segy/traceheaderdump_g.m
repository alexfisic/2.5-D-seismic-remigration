...
% traceheaderdump_g: allows interactive browsing of the SEGY trace headers from a SegyFile object
%
% traceheaderdump_g(trchdr)
%
% Program opens a figure window with 3 axes in it. Associated with each axes is a popupmenu that
% contains all of the non-empty header fields. Selecting a header field causes its values to be
% graphed. This allows interactive browsing of the headers. Any 3 header fields can be displayed
% simultansously. This program calls traceheaderdump.m to determine the non-empty header names.
%
% trchdr ... SEGY trace header structure as returned from a SegyFile object using its trace read method 
%
...
% [trchdr,trcdat]=sf.Trc.read; %read all trace headers and trace data
% traceheaderdump_g(trchdr) %pass the trace headers into traceheaderdump_g
% 
% by: G.F. Margrave, CREWES, 2017
%
...
if(strcmp(action,'init'))
    figure;
    set(gcf,'name','Trace header browser')
    [dump,allwords,inotempty]=traceheaderdump(traceheaders); %#ok<ASGLU>
    words=allwords(inotempty);
    nwords=length(words);
    
    headervals=cell(size(words));
    for k=1:nwords
        headervals{k}=getfield(traceheaders,words{k}); %#ok<*GFLD>
    end
    ...
    mm_adjust(gcf) %center fig on default monitor
...
elseif(strcmp(action,'choice1')||strcmp(action,'choice2')||strcmp(action,'choice3'))
    ...
    plot(x,headerdata)
    title([word ' minval=' num2str(hdmin) ', maxval=' num2str(hdmax) ', median separation=' num2str(hdsep)])
