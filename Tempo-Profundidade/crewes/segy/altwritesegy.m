...
if exist(sgyfile,'file')
    a = input('File exists and will be overwritten, Continue (y/n)? ','s');
    if strcmpi(a,'n')
        return
    end
end
...
%Check special cases
if isnan(datain)
    error('Input data contains NaNs; refusing to continue');
end
if isinf(datain)
    error('Input data contains Infs; refusing to continue');
end

switch segfmt
    case 1
        check_fp_range(datain,'single'); %Do not use 'ibm32'
        description = 'IBM floating point format';
        dformat = 'uint32'; % 4 bytes, should be IBM floating point
        datain = double2ibm(datain);
    case 2
        check_int_range(datain,'int32');
        description = '4-byte integer format';
        dformat = 'int32'; % 4 bytes, signed.
    case 3
        check_int_range(datain,'int16');
        description = '2-byte integer format';
        dformat = 'int16'; % 2 bytes, signed.
%     case 4
%         error('Can not write this format. (Fixed point with gain code.)');
    case 5
        description = 'IEEE floating point format';
        check_fp_range(datain,'single')
        dformat = 'float32'; % 4 bytes presumably IEEE floating point
    case 8
        check_int_range(datain,'int8');
        description = '1-byte integer format';
        dformat = 'int8'; % 4 bytes presumably IEEE floating point
    otherwise
        error(['Invalid or unsupported segfmt: ', num2str(segfmt)]);
end
