...
%Two sets of corrections are allowed - as yet specified by code in
%       fdCompB8Gilga.m
%
...
clear
parmFile = 'general.parc'; %'RaylAv.parc'; %'general.parc';
disp(parmFile)
[Dt,Dxz,pvel,svel,p2vel,s2vel,fractx,nWide,iPlTerm,wncvar] = ....
...
for iL = 1:2
    for iterm=init:last
      ...
      if iterm==1
        wncorr = (sinc(pvel*Dt*kmat)./(wx1111*sinc(Dxz*wnvectz))).^2;
        cPSdd(:,1) = 'zPzz';
      end
      if iterm==2
        wncorr = (sinc(pvel*Dt*kmat)).^2./(sinc(Dxz*wnvectx)*.....
             sinc(Dxz*wnvectz));
        cPSdd(:,2) = 'Pxz ';
      end
      if iterm==3
        wncorr = (sinc(svel*Dt*kmat)).^2./(sinc(Dxz*wnvectx)*sinc(Dxz*wnvectz));
        cPSdd(:,3) = 'Sxz ';
      end
      if iterm==4
        wncorr = (sinc(svel*Dt*kmat)./sinc(Dxz*wnvectx*wz1111)).^2;
        cPSdd(:,4) = 'zSxx';
      end
      if iterm==5
        wncorr = (sinc(pvel*Dt*kmat)./sinc(Dxz*wnvectx*wz1111)).^2;
        cPSdd(:,5) = 'xPxx';
      end
      if iterm==6
        wncorr = (sinc(svel*Dt*kmat)./sinc(Dxz*wx1111*wnvectz)).^2;
        cPSdd(:,6) = 'xSzz';
      end
...
%   atitle = [wncvar,' correction factor, Svel=',num2str(svel),.....
% 	' Pvel=',num2str(pvel),' Dz=',num2str(Dxz),' Dt=',num2str(Dt)];
        title2 = ['Svel=',num2str(svel),' Pvel=',num2str(pvel),.....
