function plotcoloura(Ux,Uz,frClip,xvect,zvect,Dxz,ibar,iblack,ampscale)
%
% Plot vector data with direction coded around the spectrum, with absolute amplitude.
%function plotcoloura(Ux,Uz,frClip,xvect,zvect,Dxz,ibar,iblack,ampscale)
%
%This plot is useful for movies because the scaling is consistent between frames.
...
%ampscale . %Translates displacement to colour intensity (100 to 300?)
%The result is clipped to 1 (most intense colours).
%
...
maxnew = maxa*ampscale;
%Allow for clipping of amplitudes>1
ampnew = amp*ampscale;
Uxnew = Ux*ampscale;
Uznew = Uz*ampscale;
if maxnew>1
...
if ibar~=0
  ...
  if iblack<=0
    Arrk(:,:,1)=1-(ampmat'-Uzmat')/(maxmat*2);
    Arrk(:,:,2)=1-(ampmat'+(.5*Uzmat'-r3o2*Uxmat'))/(maxmat*2);
    Arrk(:,:,3)=1-(ampmat'+(.5*Uzmat'+r3o2*Uxmat'))/(maxmat*2);
  else
    Arrk(:,:,1)=(ampmat'-Uzmat')/(maxmat*2);
    Arrk(:,:,2)=(ampmat'+(.5*Uzmat'-r3o2*Uxmat'))/(maxmat*2);
    Arrk(:,:,3)=(ampmat'+(.5*Uzmat'+r3o2*Uxmat'))/(maxmat*2);
  end
  %Uxnew(ibx:nx,ibz:lbz) = Uxmat*maxnew/maxmat;
  %Uznew(ibx:nx,ibz:lbz) = Uzmat*maxnew/maxmat;
  %Uxnew(ibx:nx,ibz:lbz) = Uxmat/maxmat;
  %Uznew(ibx:nx,ibz:lbz) = Uzmat/maxmat;
...
if iblack<=0
  ...
  Arr(:,:,1)=1-(ampnew'-Uznew')/2;
  Arr(:,:,2)=1-(ampnew'+(.5*Uznew'-r3o2*Uxnew'))/2;
  Arr(:,:,3)=1-(ampnew'+(.5*Uznew'+r3o2*Uxnew'))/2;
else
  Arr(:,:,1)=(ampnew'-Uznew')/(maxnew*2);
  Arr(:,:,2)=(ampnew'+(.5*Uznew'-r3o2*Uxnew'))/(maxnew*2);
  Arr(:,:,3)=(ampnew'+(.5*Uznew'+r3o2*Uxnew'))/(maxnew*2);
end
Arr(ibz:lbz,ibx:nx,:) = Arrk;
