...
% izm2 = 2:iz9;
% if nTerms > 6;
%     %izm2 = 2:134; izm3 = 135:iz9;
%     izm2 = 2:69; izm3 = 70:iz9;
% end

%Set up sample zones for correction ranges
%   Corrections designed for a given velocity zone can be applied 
%       to lower, but not higher velocity zones
nZones = round(nTerms/6);
%iTop = 2;
if nZones > 0
    iTarr = zeros(nZones);
    iBarr = zeros(nZones);
    ...
    if nZones > 1
        for iZn = 1:nZones-1
            %iBot = floor(zLvl(iZn+1)/Dxz)+iz1;
            iBot = iZlvl(iZn+1)+iz1-1;
            iBarr(iZn) = iBot;
            iTarr(iZn+1) = iBot + 1;
        end
    end
...
%     end
%     if nTerms == 12
%         h1(ixm2,izm3) = conv2(D2UxDx(ixm2,izm3),wncMat(:,:,11),'same');
%         h4(ixm2,izm3) = conv2(D2UxDz(ixm2,izm3),wncMat(:,:,12),'same');
%         h2(ixm2,izm3) = conv2(D2UzDxzP(ixm2,izm3),wncMat(:,:,8),'same');
%         h3(ixm2,izm3) = conv2(D2UzDxzS(ixm2,izm3),wncMat(:,:,9),'same');
%     end
    %Apply corrections to the X terms
    if nTerms >= 6
        iTm = 0;
        for iZn = 1:nZones
           iTop = iTarr(iZn);
           iBot = iBarr(iZn);
           h1(ixm2,iTop:iBot) = conv2(D2UxDx(ixm2,iTop:iBot),wncMat(:,:,iTm+5),'same');
           h4(ixm2,iTop:iBot) = conv2(D2UxDz(ixm2,iTop:iBot),wncMat(:,:,iTm+6),'same');
           h2(ixm2,iTop:iBot) = conv2(D2UzDxzP(ixm2,iTop:iBot),wncMat(:,:,iTm+2),'same');
           h3(ixm2,iTop:iBot) = conv2(D2UzDxzS(ixm2,iTop:iBot),wncMat(:,:,iTm+3),'same');
           iTm = iTm+6;
        end
    end
    spacex = (h1+h4+h2-h3);
...
%     end
%     if nTerms == 12
%         h1(ixm2,izm3) = conv2(D2UzDz(ixm2,izm3),wncMat(:,:,7),'same');
%         h4(ixm2,izm3) = conv2(D2UzDx(ixm2,izm3),wncMat(:,:,10),'same');
%         h2(ixm2,izm3) = conv2(D2UxDxzP(ixm2,izm3),wncMat(:,:,8),'same');
%         h3(ixm2,izm3) = conv2(D2UxDxzS(ixm2,izm3),wncMat(:,:,9),'same');
%     end
    if nTerms >= 6
        iTm = 0;
        for iZn = 1:nZones
           iTop = iTarr(iZn);
           iBot = iBarr(iZn);
            h1(ixm2,iTop:iBot) = conv2(D2UzDz(ixm2,iTop:iBot),wncMat(:,:,iTm+1),'same');
            h4(ixm2,iTop:iBot) = conv2(D2UzDx(ixm2,iTop:iBot),wncMat(:,:,iTm+4),'same');
            h2(ixm2,iTop:iBot) = conv2(D2UxDxzP(ixm2,iTop:iBot),wncMat(:,:,iTm+2),'same');
            h3(ixm2,iTop:iBot) = conv2(D2UxDxzS(ixm2,iTop:iBot),wncMat(:,:,iTm+3),'same');
            iTm = iTm+6;
        end
    end
    spacez = (h1+h4+h2-h3);
