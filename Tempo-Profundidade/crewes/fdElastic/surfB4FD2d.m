...
if strcmpi(saved,'No')
    ...
    %    'Output SEGY (K 10)');
    kO = menu('Choose computation','Load prior',.....
        'Start afresh','Continue from previous','Plot last frame (K 5)',.....
        'Plot WiggleVA (K 6)','Plot Vdensity','Plot surface at t''s','Play movie',.....
        'Make AVI movie file (K 9)','Output SEGY (K 10)');
    kOptn = kO+1;
...
%Key code 6 - Plot traces WiggleVA  **************************************
if kOptn==6
    kOut = 1;
    Getr4F(kOut)
end
%Key code 7 - Plot traces Vdensity **************************************
if kOptn==7
    kOut = 2;
    Getr4F(kOut)
end
%Key code 8 - Plot surfaces at times WiggleVA  ***************************
if kOptn==8
    kOut = 3;
    Getr4F(kOut)
end
%Key code 9 - Run Matlab movie ************************************************
if kOptn==9
    %figure
    nRepeats = 5;       %Number of times the movie is repeated, once loaded
    movie(M,nRepeats)
end
%Key code 10 - Convert movie (M) file to a movie .avi file       ***************
if kOptn==10
...
%Key code 11 - Prepare for SEGY output (intermediate text files) ********
if kOptn==11
    kOut = 9;
    Getr4F(kOut)
