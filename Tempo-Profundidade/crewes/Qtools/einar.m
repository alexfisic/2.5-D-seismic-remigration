function [quimp,tq,b]=einar(Q,x,vnot,dt,tmax,flag,frac_shift,fnot)
%EINAR: generate a constant Q impulse response
%
% [quimp,tq]=einar(Q,x,vnot,dt,tmax,flag,frac_shift)
%
...
% Q ... Q
% x ... distance traveled
% vnot ... high frequency phase velocity
% dt ... time sample rate
...
%       frac_shift=1 removed the full timme delay at computed at vnot while
%       frac_shift=.9 removes only 90% of the delay.
%   ************** default = .95 **********
% fnot ... reference frequency at which vnot is specified.
%   ************** default = .5/dt (Nyquist) *************
%
...
%amplitude spectrum
B= exp(-x*pi*abs(f)/(Q*vnot));

%phase velocity
cw=vnot./(1-log(abs(f(2:length(f)))/fnot)/(pi*Q));
cw=[cw(1) cw];
...
if(flag)
    ...
    B = B.*exp(1i*2*pi*f.*(frac_shift*x/vnot - x./cw));
end
