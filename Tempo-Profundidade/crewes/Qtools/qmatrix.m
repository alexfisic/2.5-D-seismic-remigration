function [qmat,trow]=qmatrix(q,t,w,tw,flag,flag2)
%QMATRIX ... create a matrix that applies a forward Q filter
%
% [qmat,trow]=qmatrix(q,t,w,tw,flag,flag2)
%
...
%     small drift times.
%     ************** default flag=1 ****************
% flag2 ... 1 means the Q matrix will be square of diemnsion length(t)xlength(t)
%           2 means the Q matrix has length(t)+length(w)-1 rows and length(t) columns
% **************** default flag2=1 ******************
...
if(flag~=1)
    td1=tdrift(q,x,v*ones(size(x)),fd,fnyq/2);
    td2=tdrift(q,x,v*ones(size(x)),fd,fnyq2/2);
end
